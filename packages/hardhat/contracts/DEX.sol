...
contract DEX {
    ...
    /**
     * @notice accoun address => liquidity
     */
    ...
    /**
     * @notice initializes amount of tokens that will be transferred to the DEX itself
      from the erc20 contract mintee (and only them based on how Balloons.sol is written). 
    ...
    function price(uint256 xInput, uint256 xReserves, uint256 yReserves) public pure returns (uint256 yOutput) {
        ...
        uint256 denumerator = (xReserves * 1000) + xInputWithFee;
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        require(msg.value > 0, "Not have ETH");
    ...
    function tokenToEth(uint256 _tokenInput) public returns (uint256 ethOutput) {
        require(_tokenInput > 0, "can't swap 0 token");
        require(token.balanceOf(msg.sender) >= _tokenInput, "insufficient token balance");
        require(token.allowance(msg.sender, address(this)) >= _tokenInput, "insufficient allowance");

        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 ethReserve = address(this).balance;
        ethOutput = price(_tokenInput, tokenReserve, ethReserve);
        require(token.transferFrom(msg.sender, address(this), _tokenInput), "tokenToEth(): reverted swap.");
        (bool sent, ) = msg.sender.call{ value: ethOutput }("");
        require(sent, "tokenToEth: revert in transferring eth to you!");
        emit TokenToEthSwap(msg.sender, _tokenInput, ethOutput);
        return ethOutput;
    }
